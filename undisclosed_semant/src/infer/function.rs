        ...
        let (typed, ty) = match expr.value {
            ...
            Expression::Binary {
                ...
                match op.value {
                    ...
                    Op::And | Op::Or => {
                        self.unify(&lhs.ty, &Type::App(TyCon::Bool, vec![]), span, ctx)?;
                        self.unify(&rhs.ty, &Type::App(TyCon::Bool, vec![]), span, ctx)?;
                    ...
                    Op::Plus | Op::Slash | Op::Star | Op::Minus => {
                        ...
                        if lhs.ty == Type::App(TyCon::String, vec![])
                            || rhs.ty == Type::App(TyCon::String, vec![])
                        {
                            (
                                t::Expression::Call(ctx.symbol("concat"), vec![lhs, rhs]),
                                Type::App(TyCon::String, vec![]),
                            )
                        } else {
