use tac::{Function,Temp,Program,Instruction};
...
impl <'a> Printer<'a> {
    ...
    pub fn print_instructions(&mut self,i:&Instruction) -> io::Result<()> {
        ...
        match *i {
            ...
            Instruction::Binary(ref res,ref lhs,ref op,ref rhs) => write!(&mut self.buffer,"{} <- {} {} {}",res,lhs,op,rhs),
            Instruction::Store(ref dest,ref source) => write!(&mut self.buffer,"{} <- {}",dest,source),
            Instruction::Cast(ref dest,ref sign, ref size) => write!(&mut self.buffer,"{} as {}{}",dest,sign,size),
            Instruction::Unary(ref dest,ref source,ref op) => write!(&mut self.buffer,"{} <- {}{}",dest,op,source),
            Instruction::Return(ref label) => write!(&mut self.buffer,"return @{}",label),
            Instruction::Call(ref dest,ref callee,ref args) => write!(&mut self.buffer,"{} <- call {}",dest,callee),
